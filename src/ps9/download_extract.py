...
def prepare_inputs(paths_or_urls, work_dir="data/working", extract_dir="data/extracted", max_workers=4):
    ...
    work_dir = Path(work_dir); extract_dir = Path(extract_dir)
    work_dir.mkdir(parents=True, exist_ok=True); extract_dir.mkdir(parents=True, exist_ok=True)
    staged = []

    # Downloads
    with ThreadPoolExecutor(max_workers=max_workers) as pool:
        futs = [pool.submit(download_file, u, work_dir) for u in items if is_url(u)]
        for f in as_completed(futs):
            staged.append(f.result())
    staged.extend([str(p) for p in items if not is_url(p)])
